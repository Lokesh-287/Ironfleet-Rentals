...
frappe.ui.form.on("Rental Agreement", {
  ...
  load_security_deposit_percentage(frm) {
    if (frm.security_deposit_percentage !== undefined) {
      return;
    }
  ...
  recalculate_totals(frm) {
    ...
    (frm.doc.items || []).forEach((item) => {
      ...
      const dailyRate = flt(item.daily_rate);
    ...
    const hasStart = Boolean(frm.doc.start_date);
    const hasEnd = Boolean(frm.doc.expected_end_date);
    let rentalDays = 0;
    ...
    const discountPercentage = flt(frm.doc.discount_percentage);
    const discountAmount = estimatedTotal * discountPercentage / 100;
    const netRentalTotal = estimatedTotal - discountAmount;
    const securityDepositPercentage = flt(frm.security_deposit_percentage);
    const securityDeposit = netRentalTotal * securityDepositPercentage / 100;
    const grandTotal = netRentalTotal + securityDeposit;
