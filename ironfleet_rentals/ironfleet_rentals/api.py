...
def make_rental_return(rental_agreement):
	...
	if not ra.is_fully_paid:
		return_doc.balance=ra.out_standing_amount
...
#------------------------------------------------------------------------------------------------------------------
@frappe.whitelist()
def make_final_payment(rental_agreement,rental_return,payment_mode):
	ra = frappe.get_doc("Rental Agreement", rental_agreement)
	r_return= frappe.get_doc("Rental Return", rental_return)
	if not ra.is_fully_paid:
		ra.is_fully_paid=1
		ra.out_standing_amount=0
		for item in ra.payment_schedule:
			item.status="Paid"
			item.payment_reference= payment_mode
		ra.save(ignore_permissions=True)

	r_return.is_fully_paid=1
